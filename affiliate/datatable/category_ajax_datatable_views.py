...
def category_api(request):
    try:
        ...
        categories = Category.objects.filter(global_q_object)
    ...
    try:
        if request.method == 'POST':
            ...
            if Category.objects.filter(name=name).exists():
                return JsonResponse({'success': False, 'error': 'القسم بهذا الاسم موجودة بالفعل'})
            ...
            try:
                ...
                # Handle the case where a concurrent request created the category with the same name
                return JsonResponse({'success': False, 'error': 'القسم بهذا الاسم موجودة بالفعل'})
...
def get_category(request, category_id):
    # Attempt to retrieve the Category instance or return a 404 response
    category = get_object_or_404(Category, id=category_id)

    # Create a dictionary containing the category details
    data = {
        'id': category.id,
        'name': category.name,
        'created_at': category.created_at.strftime('%Y-%m-%d %H:%M:%S'),
