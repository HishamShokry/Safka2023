...
class ProductForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["access_to"].widget.attrs.update(
            {"class": "form-control select2"}
        )
    ...
    class Meta:
        model = Product
        fields = [ 'name', 'category', 'inventory', 'vendor', 'purchase_price', 'sale_price', 'image', 'media_url', 'description', 'note', 'access_type', 'access_to', 'is_active']
...
class OrderItemForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if 'product' in self.data:
            ...
            self.fields["product"].queryset = Product.objects.filter(name__icontains=term)
        ...
        self.fields["total_item_price"].widget.attrs.update(
            {"class": "form-control", "placeholder": "اجمالي سعر الطلب"}
        )
    class Meta:
        model = OrderItem
...
class OrderForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["marketer"].widget.attrs.update(
            {"class": "form-control select2"}
        )
    ...
    class Meta:
        model = Order
        fields = ['status', 'marketer', 'client_name', 'client_phone1', 'client_phone2', 'client_address', 'governorate', 'city', 'shipping_company', 'total', 'note']  # Use all fields from the Order model
